...
const company = {
    name: 'shakil rizvi stock',
    trackId: 149,
    branches: ['shantinagar', 'motijheel','hatirjheel'],
    headOffice:{
        location: 'motijheel',
        employee: 200,
    },
    income: function(employee){
        return this.trackId*employee
    }
}

const companyIncome= company.income(300)
...
const template = `
    Hey our head office is in ${company.headOffice.location}.Plz have a visit of ${company.branches[1]} and our company name is ${company.name} 
`
...
const divideBy = number => number/7
const result = divideBy(91)
// console.log(result);
...
const multiLine = (num1 , num2)=>{
    const add7 = num1 + 7
    const added7 = num2 +7
    const result2 = add7 + added7
    return result2
}
// console.log(multiLine(14,34));
...
const arr = [1,45,12,31,24,25,14,37,54,64,91,2,7,67]

const maping = arr.map(x => x/7)
// console.log(maping);
...
/* 
array.map = gives all the element of an array in a new arry with functionality
array.forEach = similar to map..bt main difference is it doesn't return anything
array.filter = can find something with a condition in a array
array.find = can find a specific item of an array with a logic
